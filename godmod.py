...
class CodeTypingBot:
    ...
    def save_preferences(self):
        ...
        try:
            ...
            CTkMessagebox(title="Success", message="Preferences saved successfully!", icon="check")
        except Exception as e:
            CTkMessagebox(title="Error", message=f"Failed to save preferences: {str(e)}", icon="cancel")
...
Note: Restart the program if you encounter any errors'''
    ...
    def save_api_keys(self):
        ...
        try:
            ...
            CTkMessagebox(title="Success", message="API keys saved successfully!", icon="check")
        except Exception as e:
            CTkMessagebox(title="Error", message=f"Failed to save API keys: {str(e)}", icon="cancel")
            ...
            while not self.stop_typing:
                ...
                if current_clipboard_content != self.previous_clipboard_content:
                    try:
                        ...
                        CTkMessagebox(title="Error", message=f"Failed to generate code: {str(e)}", icon="cancel")
                time.sleep(0.5)
                
        except Exception as e:
            CTkMessagebox(title="Error", message=f"Failed to initialize AI model: {str(e)}", icon="cancel")
    ...
    def prepare_editor(self) -> None:
        ...
        if self.os_name == 'Darwin':
            ...
            # Fix for Windows/Linux using sequential key presses
